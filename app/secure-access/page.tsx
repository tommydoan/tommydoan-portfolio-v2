...
export default function Login() {
  ...
  useEffect(() => { 
    // Sửa lỗi: Sử dụng requestAnimationFrame để tách biệt quá trình mount
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-mono text-emerald-500">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-black tracking-tighter text-white uppercase">SECURE GATEWAY</h1>
          <p className="text-slate-600 text-[10px] mt-2 tracking-widest uppercase">Tommy Doan Portfolio Admin Access</p>
        </div>
        <form onSubmit={handleLogin} className="w-full max-w-md bg-slate-900 p-10 rounded-4xl border border-slate-800 shadow-2xl">
          ...
          <button type="submit" className="w-full bg-emerald-500 text-slate-950 font-black p-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-emerald-400 transition-colors active:scale-95">
